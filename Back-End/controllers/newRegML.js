...
const performNewML = function (req, res) {
  if (!req.body) {
    ...
    let bacteriaList = cleanList(req.body.bacteriaList, false);
...
const ML = function (
  ...
  let nameFolder, typeMeta, RFolder, scriptPath, bacterialCountFile; // Count for saving the name of the files
  ...
  async.each(
    ...
    function (err) {
      if (err) {
        ...
        console.log("@@@@@ metaFileName @@@@@"); // Modified by Shintaro Kinoshita
        console.log(metaFileName);               // Modified by Shintaro Kinoshita
        ...
        commandLine(title, args, function (err) {
          if (err) {
            ...
            if (newModel) {
              ...
              if (typeMeta === "bacterialCount") {
                content =
                  '"2", "2\\\\ RMSE: ' +
                  contentForTrainModel.RMSE +
                  " \\\\ Acc: " +
                  contentForTrainModel.Acc +
                  "% \\\\ Delta: " +
                  contentForTrainModel.Delta +
                  " \\\\ Af: " +
                  contentForTrainModel.Af +
                  " \\\\ Bf: " +
  ...
  async.parallel(
    {
      ...
      part2: function (callback) {
        setTimeout(function () {
          if (type === "bacterialCount") {
            readFileForLatex(
              RFolder + "HEATMAPS/rankRmse.csv",
              "Ranking of machine methods by RMSE in first place and accuracy in second place.",
              false,
      ...
      part3: function (callback) {
        setTimeout(function () {
          ...
          if (type != "bacterialCount") {
            ...
            try {
              let path = RFolder + "PREDICTIONS/Confusion_Matrix/";
...
const createPlatformObj = (arg_platname, arg_platpath, arg_metapath, arg_bacterialname) => {
  var platformList = [];
...
const makeJsonConfig = function (
  ...
  // Make 'log' into boolean type from String
  if (log == "TRUE") {log = true; }
  else               {log = false;}

  // Show logs
  console.log("mlmList       : " + mlmList);
  console.log("bacteriaList  : " + bacteriaList);
  console.log("platformsList : " + platformsList);
  console.log("convertLog    : " + log);
  ...
  var templateJSON = {
    name: null,                   // Title of JSON
    outputDirectory: null,        // Output directory path
    createStatisticsFile: null,   // If create Statistics files, default true
    createPerformancePlots: null, // If create performance plots, default true
    createPCAPlots: null,         // If create PCA plots, default true
    convertLog : null,            // If metadata (e.g. bacterial count) is log-converted, default false
    platformList : [],            // Platform list
    machineLearningModels : []    // ML models list
  };
  ...
  templateJSON["convertLog"] = log;

  // Add "platformList" property into template JSON
  templateJSON["platformList"] = createPlatformObj(platformsList, savedFileNames, metaFileName, bacteriaList);
