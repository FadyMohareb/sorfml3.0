 ...
 */
 const getResultNew = function (req, res) {
  if (req.params.path == "" || !req.params.path) {
    ...
    let path = req.params.path + "HEATMAPS/rankRmse.csv";
    let n = 0;
    let rank = [];
    ...
    lineReader.on("line", function (line) {
      if (n === 0) {
        ...
        if (splitLine.length >= 10) { // Check if there are enough elements in the split line
          // Get the values from the split line
          let rankValue = splitLine[0].trim();
          let method = splitLine[1].trim();
          let platform = splitLine[2].trim();
          let bacteria = splitLine[3].trim();
          let RMSE = parseFloat(splitLine[4].trim());
          let Acc = parseFloat(splitLine[5].trim()); // Assuming Acc is a float
          let Delta = parseFloat(splitLine[6].trim());
          let Af = parseFloat(splitLine[7].trim());
          let Bf = parseFloat(splitLine[8].trim());
          let k = parseInt(splitLine[9].trim());
